...
overlap.v2 <- intersect(overlap.GSE78220, overlap.MGSP)
...
data1.v2 <- GSE78220_matrix[,c('Lable', overlap.v2)]
...
setwd("F:/Desktop2_clone/Donghai_Desktop2/NCpaper_argument_discuss/Redo_analysis/For_testAUC/LogReg_self_fromGSE78220/")
...
p <- p[,dim(p)[2]]
pred_Bridge_forMG <- cbind(y.test2, p)   



#######################################################################################

### for data4_PRJEB23709
...
overlap.v3 <- intersect(overlap.GSE78220, overlap.PRJEB23709)
...
data1.v3 <- GSE78220_matrix[,c('Lable', overlap.v3)]
...
data4 <- PRJEB23709_matrix[,c('Lable', overlap.v3)]
...
pred_ridge2_forPRJ <- cbind(y.test2, p2)  
...
overlap.v1 <- intersect(overlap.GSE78220, overlap.GSE91061)
...
data1 <- GSE78220_matrix[,c('Lable', overlap.v1)]
...
data2 <- GSE91061_matrix[,c('Lable', overlap.v1)]
...
p <- predict(Bridge.model, "prob", new.x.mat = x.test2)
p <- p[,dim(p)[2]]
pred_Bridge_for91 <- cbind(y.test2, p)   
...
MGSP.ImSig.mx.V2 <- MGSP.ImSig.mx.V2[,colnames(GSE78220.ImSig.mx.V2)]
...
PRJEB23709.ImSig.mx.V2 <- PRJEB23709.ImSig.mx.V2[,colnames(GSE78220.ImSig.mx.V2)]
...
GSE91061.ImSig.mx.V2 <- GSE91061.ImSig.mx.V2[,colnames(GSE78220.ImSig.mx.V2)]
...
common_genes <- colnames(GSE78220.ImSig.mx.V2)[-1]
...
all(rownames(GSE78220.pca)==rownames(MGSP.pca))


merged <- cbind(GSE78220.pca, MGSP.pca)
...
merged.lable <- c(GSE78220.ImSig.mx.V2$Lable, MGSP.ImSig.mx.V2$Lable)

all(rownames(merged.t)==c(rownames(GSE78220.ImSig.mx.V2), rownames(MGSP.ImSig.mx.V2)))
...
group_list <- c(rep('GSE78220', dim(GSE78220.pca)[2]), rep('MGSP', dim(MGSP.pca)[2]))
...
all(colnames(merged) == c(rownames(GSE78220.ImSig.mx.V2), rownames(MGSP.ImSig.mx.V2)))
...
GSE78220_MGSP_batch_corrected <- removeBatchEffect(t(dat.pca$call$X), batch=group_list)

dim(GSE78220_MGSP_batch_corrected)
head(GSE78220_MGSP_batch_corrected)


dat.pca.V2 <- PCA(t(GSE78220_MGSP_batch_corrected), graph = FALSE, scale.unit = TRUE) 
...
all(dat.pca.V2$call$X == t(GSE78220_MGSP_batch_corrected))
...
GSE78220.with.impute <- t(GSE78220_MGSP_batch_corrected)[rownames(GSE78220.ImSig.mx.V2),]
...
MGSP.with.impute <- t(GSE78220_MGSP_batch_corrected)[rownames(MGSP.ImSig.mx.V2),]
...
pred_elastic_bc_forMG <- cbind(y.test, p)   #temp按行和pred合并
...
all(rownames(GSE78220.pca)==rownames(PRJEB23709.pca))


merged <- cbind(GSE78220.pca, PRJEB23709.pca)
...
merged.lable <- c(GSE78220.ImSig.mx.V2$Lable, PRJEB23709.ImSig.mx.V2$Lable)

all(rownames(merged.t)==c(rownames(GSE78220.ImSig.mx.V2), rownames(PRJEB23709.ImSig.mx.V2)))
...
group_list <- c(rep('GSE78220', dim(GSE78220.pca)[2]), rep('PRJEB23709', dim(PRJEB23709.pca)[2]))
dat.pca <- PCA(t(merged.t.impute.V2), graph = FALSE, scale.unit = TRUE) 
g<-fviz_pca_ind(dat.pca,repel =T,
                geom.ind = "point", # show points only (nbut not "text")只显示点不显示文本
                col.ind =  group_list, # color by groups 颜色组
...
all(colnames(merged) == c(rownames(GSE78220.ImSig.mx.V2), rownames(PRJEB23709.ImSig.mx.V2)))
...
GSE78220_PRJEB23709_batch_corrected <- removeBatchEffect(t(dat.pca$call$X), batch=group_list)

dim(GSE78220_PRJEB23709_batch_corrected)
head(GSE78220_PRJEB23709_batch_corrected)


dat.pca.V2 <- PCA(t(GSE78220_PRJEB23709_batch_corrected), graph = FALSE, scale.unit = TRUE) 
g<-fviz_pca_ind(dat.pca.V2,repel =T,
                geom.ind = "point", 
                col.ind =  group_list, 
                # palette = c("#00AFBB", "#E7B800"),
...
all(dat.pca.V2$call$X == t(GSE78220_PRJEB23709_batch_corrected))
...
GSE78220.with.impute <- t(GSE78220_PRJEB23709_batch_corrected)[rownames(GSE78220.ImSig.mx.V2),]
...
PRJEB23709.with.impute <- t(GSE78220_PRJEB23709_batch_corrected)[rownames(PRJEB23709.ImSig.mx.V2),]
...
pred_lasso_bc_forPRJ <- cbind(y.test, p)   
...
all(rownames(GSE78220.pca)==rownames(GSE91061.pca))


merged <- cbind(GSE78220.pca, GSE91061.pca)
...
merged.lable <- c(GSE78220.ImSig.mx.V2$Lable, GSE91061.ImSig.mx.V2$Lable)

all(rownames(merged.t)==c(rownames(GSE78220.ImSig.mx.V2), rownames(GSE91061.ImSig.mx.V2)))
...
group_list <- c(rep('GSE78220', dim(GSE78220.pca)[2]), rep('GSE91061', dim(GSE91061.pca)[2]))
dat.pca <- PCA(t(merged.t.impute.V2), graph = FALSE, scale.unit = TRUE) 
g<-fviz_pca_ind(dat.pca,repel =T,
                geom.ind = "point", # show points only (nbut not "text")只显示点不显示文本
                col.ind =  group_list, # color by groups 颜色组
...
all(colnames(merged) == c(rownames(GSE78220.ImSig.mx.V2), rownames(GSE91061.ImSig.mx.V2)))
...
GSE78220_GSE91061_batch_corrected <- removeBatchEffect(t(dat.pca$call$X), batch=group_list)

dim(GSE78220_GSE91061_batch_corrected)
head(GSE78220_GSE91061_batch_corrected)


dat.pca.V2 <- PCA(t(GSE78220_GSE91061_batch_corrected), graph = FALSE, scale.unit = TRUE) 
g<-fviz_pca_ind(dat.pca.V2,repel =T,
                geom.ind = "point", 
                col.ind =  group_list, 
                # palette = c("#00AFBB", "#E7B800"),
...
all(dat.pca.V2$call$X == t(GSE78220_GSE91061_batch_corrected))
...
GSE78220.with.impute <- t(GSE78220_GSE91061_batch_corrected)[rownames(GSE78220.ImSig.mx.V2),]
...
GSE91061.with.impute <- t(GSE78220_GSE91061_batch_corrected)[rownames(GSE91061.ImSig.mx.V2),]
...
p <- p[,dim(p)[2]]
pred_Bridge_bc_for91 <- cbind(y.test, p)   
...
jpeg(file = "Fig2g_from78_wo_or_with_bc_multiple_plots.jpg")

roc_wo_bc_MGSP <- plot.roc(pred_Bridge_forMG[,1], pred_Bridge_forMG[,2], main = "ROC curves", add =  FALSE, asp = NA, print.auc =F, col = colset[1])

auc1=round(as.numeric(gsub('Area under the curve: ','',roc_wo_bc_MGSP$auc)),digits=3)


roc_wo_bc_PRJ <- roc(pred_ridge2_forPRJ[,1], pred_ridge2_forPRJ[,2], main = "Smoothing")

auc2=round(as.numeric(gsub('Area under the curve: ','',roc_wo_bc_PRJ$auc)),digits=3)


roc_wo_bc_GSE91061 <- roc(pred_Bridge_for91[,1], pred_Bridge_for91[,2], main = "Smoothing")

auc3=round(as.numeric(gsub('Area under the curve: ','',roc_wo_bc_GSE91061$auc)),digits=3)


roc_with_bc_MGSP <- roc(pred_elastic_bc_forMG[,1], pred_elastic_bc_forMG[,2], main = "Smoothing")

auc4=round(as.numeric(gsub('Area under the curve: ','',roc_with_bc_MGSP$auc)),digits=3)


roc_with_bc_PRJ <- roc(pred_lasso_bc_forPRJ[,1], pred_lasso_bc_forPRJ[,2], main = "Smoothing")

auc5=round(as.numeric(gsub('Area under the curve: ','',roc_with_bc_PRJ$auc)),digits=3)


roc_with_bc_GSE91061 <- roc(pred_Bridge_bc_for91[,1], pred_Bridge_bc_for91[,2], main = "Smoothing")

auc6=round(as.numeric(gsub('Area under the curve: ','',roc_with_bc_GSE91061$auc)),digits=3)


leg1 = paste0("MGSP wo BC: AUC = ", auc1)
leg2 = paste0("PRJEB23709 wo BC: AUC = ", auc2)
leg3 = paste0("GSE91061 wo BC: AUC = ", auc3)

leg4 = paste0("MGSP with BC: AUC = ", auc4)
leg5 = paste0("PRJEB23709 with BC: AUC = ", auc5)
leg6 = paste0("GSE91061 with BC: AUC = ", auc6)


lines(roc_wo_bc_PRJ, type = "l", lty = 3, col = colset[2])
lines(roc_wo_bc_GSE91061, type = "l", lty = 4, col = colset[3])
lines(roc_with_bc_MGSP, type = "l", lty = 1, col = colset[6])
lines(roc_with_bc_PRJ, type = "l", lty = 4, col = 'blue',lwd = 4)
