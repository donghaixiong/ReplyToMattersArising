...
overlap.v1 <- intersect(overlap.PRJEB23709, overlap.GSE78220)
...
data1 <- PRJEB23709_matrix[,c('Lable', overlap.v1)]
...
x.test2 <- model.matrix(Lable ~., data2)[,-1] 

y.test2 <- data2$Lable


setwd("F:/Desktop2_clone/Donghai_Desktop2/NCpaper_argument_discuss/Redo_analysis/For_testAUC/LogReg_self_fromPRJEB23709/")
...
p <- predict(Bridge.model, "prob", new.x.mat = x.test2)
p <- p[,dim(p)[2]]
pred_Bridge <- cbind(y.test2, p)   
...
overlap.v2 <- intersect(overlap.PRJEB23709, overlap.MGSP)
...
data1.v2 <- PRJEB23709_matrix[,c('Lable', overlap.v2)]
...
data3 <- MGSP_matrix[,c('Lable', overlap.v2)]
...
overlap.v3 <- intersect(overlap.PRJEB23709, overlap.GSE91061)
...
data1.v3 <- PRJEB23709_matrix[,c('Lable', overlap.v3)]
...
data4 <- GSE91061_matrix[,c('Lable', overlap.v3)]
...
x.test2 <- model.matrix(Lable ~., data4)[,-1] 
...
p <- predict(SCAD.model, x.test2, type = "response")
pred_SCAD <- cbind(y.test2, p)   
...
GSE78220.ImSig.mx.V2 <- GSE78220.ImSig.mx.V2[,colnames(PRJEB23709.ImSig.mx.V2)]
...
overlap.NatMed.MGSP <- intersect(colnames(NatMed_103samples_GE_matrix), ImSig)

length(overlap.NatMed.MGSP)

overlap.NatMed.MGSP

missing.MGSP <- setdiff(ImSig, overlap.NatMed.MGSP)

missing.MGSP

MGSP.ImSig.mx <- NatMed_103samples_GE_matrix[,c('Lable',overlap.NatMed.MGSP)]
...
MGSP.ImSig.mx.V2 <- MGSP.ImSig.mx.V2[,colnames(PRJEB23709.ImSig.mx.V2)]
...
GSE91061.ImSig.mx.V2 <- GSE91061.ImSig.mx.V2[,colnames(PRJEB23709.ImSig.mx.V2)]
...
common_genes <- colnames(PRJEB23709.ImSig.mx.V2)[-1]
...
all(rownames(PRJEB23709.pca)==rownames(GSE78220.pca))


merged <- cbind(PRJEB23709.pca, GSE78220.pca)
...
merged.lable <- c(PRJEB23709.ImSig.mx.V2$Lable, GSE78220.ImSig.mx.V2$Lable)

all(rownames(merged.t)==c(rownames(PRJEB23709.ImSig.mx.V2), rownames(GSE78220.ImSig.mx.V2)))
...
group_list <- c(rep('PRJEB23709', dim(PRJEB23709.pca)[2]), rep('GSE78220', dim(GSE78220.pca)[2]))
dat.pca <- PCA(t(merged.t.impute.V2), graph = FALSE, scale.unit = TRUE) #'-'表示“非”
...
all(colnames(merged) == c(rownames(PRJEB23709.ImSig.mx.V2), rownames(GSE78220.ImSig.mx.V2)))
...
PRJEB23709_GSE78220_batch_corrected <- removeBatchEffect(t(dat.pca$call$X), batch=group_list)

dim(PRJEB23709_GSE78220_batch_corrected)
head(PRJEB23709_GSE78220_batch_corrected)


dat.pca.V2 <- PCA(t(PRJEB23709_GSE78220_batch_corrected), graph = FALSE, scale.unit = TRUE) 
...
all(dat.pca.V2$call$X == t(PRJEB23709_GSE78220_batch_corrected))
...
PRJEB23709.with.impute <- t(PRJEB23709_GSE78220_batch_corrected)[rownames(PRJEB23709.ImSig.mx.V2),]
...
GSE78220.with.impute <- t(PRJEB23709_GSE78220_batch_corrected)[rownames(GSE78220.ImSig.mx.V2),]
...
pred_lasso_withBC_for78 <- cbind(y.test, p)   #temp按行和pred合并
...
all(rownames(PRJEB23709.pca)==rownames(MGSP.pca))


merged <- cbind(PRJEB23709.pca, MGSP.pca)
...
merged.lable <- c(PRJEB23709.ImSig.mx.V2$Lable, MGSP.ImSig.mx.V2$Lable)

all(rownames(merged.t)==c(rownames(PRJEB23709.ImSig.mx.V2), rownames(MGSP.ImSig.mx.V2)))
...
group_list <- c(rep('PRJEB23709', dim(PRJEB23709.pca)[2]), rep('MGSP', dim(MGSP.pca)[2]))
...
all(colnames(merged) == c(rownames(PRJEB23709.ImSig.mx.V2), rownames(MGSP.ImSig.mx.V2)))
...
PRJEB23709_MGSP_batch_corrected <- removeBatchEffect(t(dat.pca$call$X), batch=group_list)

dim(PRJEB23709_MGSP_batch_corrected)
head(PRJEB23709_MGSP_batch_corrected)


dat.pca.V2 <- PCA(t(PRJEB23709_MGSP_batch_corrected), graph = FALSE, scale.unit = TRUE) 
g<-fviz_pca_ind(dat.pca.V2,repel =T,
                geom.ind = "point", 
                col.ind =  group_list, 
                # palette = c("#00AFBB", "#E7B800"),
...
all(dat.pca.V2$call$X == t(PRJEB23709_MGSP_batch_corrected))
...
PRJEB23709.with.impute <- t(PRJEB23709_MGSP_batch_corrected)[rownames(PRJEB23709.ImSig.mx.V2),]
...
MGSP.with.impute <- t(PRJEB23709_MGSP_batch_corrected)[rownames(MGSP.ImSig.mx.V2),]
...
p <- p[,dim(p)[2]]
pred_Bridge_BC_forMG <- cbind(y.test, p)   
...
all(rownames(PRJEB23709.pca)==rownames(GSE91061.pca))


merged <- cbind(PRJEB23709.pca, GSE91061.pca)
...
merged.lable <- c(PRJEB23709.ImSig.mx.V2$Lable, GSE91061.ImSig.mx.V2$Lable)

all(rownames(merged.t)==c(rownames(PRJEB23709.ImSig.mx.V2), rownames(GSE91061.ImSig.mx.V2)))
...
group_list <- c(rep('PRJEB23709', dim(PRJEB23709.pca)[2]), rep('GSE91061', dim(GSE91061.pca)[2]))
dat.pca <- PCA(t(merged.t.impute.V2), graph = FALSE, scale.unit = TRUE) 
g<-fviz_pca_ind(dat.pca,repel =T,
...
all(colnames(merged) == c(rownames(PRJEB23709.ImSig.mx.V2), rownames(GSE91061.ImSig.mx.V2)))
...
PRJEB23709_GSE91061_batch_corrected <- removeBatchEffect(t(dat.pca$call$X), batch=group_list)

dim(PRJEB23709_GSE91061_batch_corrected)
head(PRJEB23709_GSE91061_batch_corrected)


dat.pca.V2 <- PCA(t(PRJEB23709_GSE91061_batch_corrected), graph = FALSE, scale.unit = TRUE) 
g<-fviz_pca_ind(dat.pca.V2,repel =T,
                geom.ind = "point", 
                col.ind =  group_list, 
                # palette = c("#00AFBB", "#E7B800"),
...
all(dat.pca.V2$call$X == t(PRJEB23709_GSE91061_batch_corrected))
...
PRJEB23709_GSE91061_batch_corrected <- removeBatchEffect(merged, group_list)
...
PRJEB23709.with.impute <- t(PRJEB23709_GSE91061_batch_corrected)[rownames(PRJEB23709.ImSig.mx.V2),]
...
GSE91061.with.impute <- t(PRJEB23709_GSE91061_batch_corrected)[rownames(GSE91061.ImSig.mx.V2),]
...
pred_ridge_BC_for91 <- cbind(y.test, p)   
...
jpeg(file = "Fig2f_fromPRJ_wo_or_with_bc_multiple_plots.jpg")

roc_wo_bc_GSE78220 <- plot.roc(pred_Bridge[,1], pred_Bridge[,2], main = "ROC curves", add =  FALSE, asp = NA, print.auc =F, col = colset[1])

auc1=round(as.numeric(gsub('Area under the curve: ','',roc_wo_bc_GSE78220$auc)),digits=3)


roc_wo_bc_MGSP <- roc(pred_ridge2[,1], pred_ridge2[,2], main = "Smoothing")

auc2=round(as.numeric(gsub('Area under the curve: ','',roc_wo_bc_MGSP$auc)),digits=3)


roc_wo_bc_GSE91061 <- roc(pred_SCAD[,1], pred_SCAD[,2], main = "Smoothing")

auc3=round(as.numeric(gsub('Area under the curve: ','',roc_wo_bc_GSE91061$auc)),digits=3)


roc_with_bc_GSE78220 <- roc(pred_lasso_withBC_for78[,1], pred_lasso_withBC_for78[,2], main = "Smoothing")

auc4=round(as.numeric(gsub('Area under the curve: ','',roc_with_bc_GSE78220$auc)),digits=3)


roc_with_bc_MGSP <- roc(pred_Bridge_BC_forMG[,1], pred_Bridge_BC_forMG[,2], main = "Smoothing")

auc5=round(as.numeric(gsub('Area under the curve: ','',roc_with_bc_MGSP$auc)),digits=3)


roc_with_bc_GSE91061 <- roc(pred_ridge_BC_for91[,1], pred_ridge_BC_for91[,2], main = "Smoothing")

auc6=round(as.numeric(gsub('Area under the curve: ','',roc_with_bc_GSE91061$auc)),digits=3)


leg1 = paste0("GSE78220 wo BC: AUC = ", auc1)
leg2 = paste0("MGSP wo BC: AUC = ", auc2)
leg3 = paste0("GSE91061 wo BC: AUC = ", auc3)


leg4 = paste0("GSE78220 with BC: AUC = ", auc4)
leg5 = paste0("MGSP with BC: AUC = ", auc5)
leg6 = paste0("GSE91061 with BC: AUC = ", auc6)


lines(roc_wo_bc_MGSP, type = "l", lty = 3, col = colset[2])
lines(roc_wo_bc_GSE91061, type = "l", lty = 4, col = colset[3])
lines(roc_with_bc_GSE78220, type = "l", lty = 1, col = colset[6])
lines(roc_with_bc_MGSP, type = "l", lty = 4, col = 'blue',lwd = 4)
lines(roc_with_bc_GSE91061, type = "l", lty = 1, col = 'black')
